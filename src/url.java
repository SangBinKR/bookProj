	...
	public static void main(String[] args) {
		...
		Thread th = new Thread() {
			public void run() {
				try {
					// 다운로드 받을 주소 만들기
					String addr = "https://ridibooks.com/category/books/100";
...
//					 System.out.println(html);
					...
					// 클래스가 title인 데이터 전부 찾아오기
					Elements title = doc.getElementsByClass("title_text");
					if(title != null) {
						for(int i=0;i<title.size();i++){
							bookTitleList.add(title.get(i).text());
						}
						for(String s : bookTitleList){
