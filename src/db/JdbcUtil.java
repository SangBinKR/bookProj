...
public class JdbcUtil {
	...
	public static Connection getConnection() {
		...
		try {
			...
			DataSource ds = (DataSource)envCtx.lookup("jdbc/MySQL"); // 나중에 얘만 바뀌면된다. 다른 작업시
			
			// DataSource 객체의 getConnection() 메서드를 호출하여 Connection 객체 가져오기
			con = ds.getConnection(); 
			// => ds.getConnection(username, password) 형식으로 사용할 수도 있다!
			
			// JDBC 를 통한 DB작업에 대한 Auto Commit 기능 해제
			// => 트랜젝션 개념을 적용하기 위함 (작업의단위) 성공했다면 commit할거고 
			// exception발생하면 rollback해서 돌릴거임
			// commit메서드 rollback메서드 만들어준다.
	...
	//	  작업 실패시 rollback() 메서드에서 Connection 객체의 rollback() 메서드를
	// => 파라미터 : Connectin r객체, 리턴타입 :void
